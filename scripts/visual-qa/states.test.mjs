...
import { VISUAL_QA_STATES, getVisualQaState } from './states.mjs';
...
test('exports the exact 13-state capture manifest', () => {
  assert.equal(VISUAL_QA_STATES.length, 13);
  assert.deepEqual(VISUAL_QA_STATES, EXPECTED_STATES);
  assert.equal(new Set(VISUAL_QA_STATES.map(({ id }) => id)).size, 13);
});

test('returns only whitelisted visual QA states', () => {
  assert.deepEqual(getVisualQaState('practice'), EXPECTED_STATES[0]);
  assert.throws(
    () => getVisualQaState('unknown-state'),
    /Unknown visual QA state: unknown-state/
  );
});
